...
export const shadowProperty: Property = {
  shadow: ({ key = '', value = '', secondValue = '', secondUnit = '' }) => {
    ...
    if (is.color.test(value))
      return `value:--anc-${shadowType}-color: ${createColor(
        value,
        secondValue,
        secondUnit
      )}` as GetCSSProperty
    else
      return `value:--anc-${shadowType}: ${
        (!value ? values['sm'] : key === 'inset' ? insetValues[value] : values[value]) || value
  ...
  ring: ({ key = '', value = '', unit = '', secondValue = '', secondUnit = '' }) => {
    ...
    if (is.color.test(value) || value === 'current')
      return `value:--anc-${shadowType}-color: ${
